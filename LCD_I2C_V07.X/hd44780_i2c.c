...
void LCD_Command(uint8_t cmd) {
    ...
    PCF8574_LCD_SendNibble(cmd >> 4, LCD_CMD);     // High nibble
    PCF8574_LCD_SendNibble(cmd & 0x0F, LCD_CMD);   // Low nibble
    __delay_ms(2);
}

void LCD_Data(uint8_t data) {
    // PCF8574_LCD_SendByte(data, 1);
    PCF8574_LCD_SendNibble(data >> 4, LCD_DATA);     // High nibble
    PCF8574_LCD_SendNibble(data & 0x0F, LCD_DATA);   // Low nibble
    __delay_ms(2);
}
